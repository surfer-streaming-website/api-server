...
//    List<SongEntity> findKeyword1(String keyword);
    ...
    @Query("SELECT DISTINCT new com.surfer.apiserver.api.search.dto.SongSearchDTO(ae.albumImage, se.songTitle, sse.songSingerName,se.songSeq,ae.albumState) FROM SongSingerEntity sse join sse.songEntity se join se.albumEntity ae WHERE se.songTitle LIKE %:keyword% OR sse.songSingerName LIKE %:keyword%")
    List<SongSearchDTO> findKeywordSong(@Param("keyword") String keyword);

    @Query("SELECT DISTINCT new com.surfer.apiserver.api.search.dto.AlbumSearchDTO(ae.albumImage,ae.albumTitle, ase.albumSingerName ,ae.albumSeq,ae.albumState) FROM AlbumSingerEntity ase join ase.albumEntity ae WHERE ase.albumSingerName LIKE %:keyword%")
    List<AlbumSearchDTO> findKeywordAlbum(@Param("keyword") String keyword);

    @Query("SELECT new com.surfer.apiserver.api.search.dto.PlaylistSearchDTO(ple.playlistName, ae.albumImage, me.name , ple.playlistGroupSeq,ae.albumState) FROM PlaylistTrackEntity plte JOIN plte.playlistGroupEntity ple JOIN ple.memberEntity me JOIN plte.songEntity se JOIN se.albumEntity ae WHERE ple.playlistName LIKE %:keyword% AND plte.regDate = (SELECT MIN(plte2.regDate) FROM PlaylistTrackEntity plte2 WHERE plte2.playlistGroupEntity = ple)")
    List<PlaylistSearchDTO> findKeywordPlaylist(@Param("keyword") String keyword);

     @Query("SELECT DISTINCT new com.surfer.apiserver.api.search.dto.LyricsSearchDTO(ae.albumImage, se.songTitle, sse.songSingerName, se.lyrics,se.songSeq,ae.albumState) FROM SongSingerEntity sse join sse.songEntity se join se.albumEntity ae WHERE se.lyrics LIKE %:keyword%")
